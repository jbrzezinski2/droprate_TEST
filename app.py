...
.stCheckbox [data-testid="stCheckbox"] { gap: 8px !important; }
...
with filter_col:
    ...
    <div style='text-align:right;font-size:12px;color:#64748b;margin-bottom:8px'>
        🟢 LIVE &nbsp;|&nbsp; {db_stats['games']:,} gier
    </div>
    """, unsafe_allow_html=True)

    st.markdown("<div class='filter-panel-title'>FILTRY</div>", unsafe_allow_html=True)

    # ── Gatunki (checkboxy jak Steam) ─────────────────────────────────────────
    with st.expander("GATUNKI", expanded=True):
        genre_checks = {}
        for genre in ALL_GENRES:
            checked = genre in st.session_state.selected_genres
            genre_checks[genre] = st.checkbox(genre, value=checked, key=f"chk_{genre}")

    # ── Zastosuj / Reset ──────────────────────────────────────────────────────
    st.write("")
    if st.button("✅ Zastosuj", use_container_width=True):
        st.session_state.selected_genres = [g for g, v in genre_checks.items() if v]
    ...
    if sel_count < len(ALL_GENRES):
        st.caption(f"🔍 {sel_count}/{len(ALL_GENRES)} gatunków")
...
with main_col:
    ...
    with tab1:
        ...
        with col_l:
            ...
            if not genre_df.empty:
                sel_genre = st.selectbox("Podświetl gatunek", ["Wszystkie"] + genre_df["genre"].tolist(), key="hl")
                df_plot = genre_df.head(10).copy()
                df_plot["kolor"] = df_plot["genre"].apply(
                    lambda x: "Wybrany" if (sel_genre != "Wszystkie" and x == sel_genre) else "Pozostałe")
                fig = px.bar(df_plot, x="roi_score", y="genre", orientation="h",
                    color="kolor" if sel_genre != "Wszystkie" else "roi_score",
    ...
    with tab2:
        ...
        with col1:
            ...
            metric_opt = st.selectbox("Metryka", ["avg_owners","avg_revenue","avg_review"], key="tm",
            ...
            if not trend_df.empty and "recorded_at" in trend_df.columns:
                fig = px.line(trend_df, x="recorded_at", y=metric_opt, color="genre",
        ...
        with col2:
            ...
            sort_by = st.selectbox("Sortuj wg", ["avg_review","roi_score","avg_revenue"], key="gs",
                format_func=lambda x: {"avg_review":"Review","roi_score":"ROI","avg_revenue":"Przychód"}[x])
            if not genre_df.empty:
                df_s = genre_df.sort_values(sort_by, ascending=False).head(8)
                fig2 = px.bar(df_s, x="genre", y=sort_by, color=sort_by,
                    color_continuous_scale=[[0,"#312e81"],[1,"#6366f1"]],
                    text=df_s[sort_by].round(1))
                fig2.update_layout(**dark_layout(height=280, coloraxis_showscale=False,
        ...
        heat_data = np.array([heat_all[g] for g in sel_h])
    ...
    with tab3:
        ...
        if not genre_df.empty:
            g_sel = st.selectbox("Wybierz gatunek", genre_df["genre"].tolist(), key="gd")
            row = genre_df[genre_df["genre"] == g_sel].iloc[0]
